    ...
    <div class="content">
        <div class="page-title-box">
            <div class="row align-items-center">
                <div class="col-sm-6">
                    <h4 class="page-title">{{ __('admin.employments') }}</h4>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-right">
                        <li class="breadcrumb-item"><a href="{{ route('adminhome') }}">{{ __('admin.home') }}</a></li>
                        <li class="breadcrumb-item active">{{ __('admin.employments') }}</li>
                    </ol>
                </div>
            </div> <!-- end row -->
            <br>
            <div class="row align-items-center">
                ...
                <div class="text-center">
                    <!-- Large modal -->
                    @if (Auth::guard('admin')->user()->hasPermission('employments-create'))
                        <button type="button" class="btn btn-primary waves-effect waves-light" data-toggle="modal"
                            data-target="#modal-add-employment" style="margin-right: 20px;margin-left: 20px;"><i
                                class="fas fa-plus-circle" style="padding-inline-end: 5px;"></i>{{ __('admin.add') }}
                            {{ __('admin.employment') }}</button>
                    @endif
                    <button type="button" class="btn btn-primary reload float-right mb-3"> <i class="fas fa-sync-alt"></i></button>
                </div>

                <!--  Modal content for the above example -->
                <div class="modal fade modal-add-employment" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
                    aria-hidden="true" id="modal-add-employment">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="myLargeModalLabel">{{ __('admin.add') }}
                                    {{ __('admin.employment') }}</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"
                                    data-target="#modal-add-employment">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>

                            <div class="modal-body" id="modal-add-employment">
                                <div class="modal-body">
                                    <div class="card-body">
                                        <form id="addemployment" method="POST" enctype="multipart/form-data" data-parsley-validate>
                                            @csrf
                                            @method('post')
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-10">
                                                    <input class="form-control .inp8" value="{{ old('url') }}"
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-10">
                                                 <textarea class="form-control .inp3"  name="description_ar" id="editor4" cols="10" rows="10"></textarea>
                                                 <script>
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-10">
                                                 <textarea class="form-control .inp4"  name="description_en" id="editor5" cols="10" rows="10"></textarea>
                                                <script>
...
@push('js')
    ...
    <script>
        $('body').on('click', '#check', function() {
            ...
            var employment_id = $(this).data('id');
            // alert(employment_id);
            $.ajax({
                url: '{{ route('employments.status') }}',
                type: 'GET',
                data: {
                    'active': active,
    ...
    <script>
            ...
            $('body').on('submit', '#addemployment', function(e) {
                        e.preventDefault();
                        if ($('.inp1').is(':empty') || $('.inp2').is(':empty') || $('.inp8').is(':empty') || $('.inp3').is(':empty') || $('.inp4').is(':empty')){
                        }
            else {
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.status == 'success') {
                            ...
                            $("#addemployment").trigger("reset");
                            $('#modal-add-employment .close').click();
